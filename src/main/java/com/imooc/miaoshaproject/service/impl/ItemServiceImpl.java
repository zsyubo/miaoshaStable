...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemModel getItemByIdInCache(Integer id) {
        Object obj = redisTemplate.opsForValue().get("_item_validate_" + id);
        ItemModel itemModel = null;
        if (obj == null) {
            itemModel = this.getItemById(id);
            // 如果保证原子性
            redisTemplate.opsForValue().set("_item_validate_" + id, itemModel);
            redisTemplate.expire("_item_validate_" + id, 10, TimeUnit.MINUTES);
        } else {
...
//            redisTemplate.opsForValue().increment("promo_item_stock_"+itemId, amount.intValue() );
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public StockLogDO initStockLog(Integer itemId, Integer amount) {
